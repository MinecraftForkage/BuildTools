...
public class CreateDiff2Task extends Task {
	...
	private static class IntCorpusIndex {
		...
		public int old_search(int[] needle, int needleStart, int needleLength) {
			//outer: for(int k = 0; k <= haystack.length - needleLength; k++) {
			...
			outer: for(int k : indicesToTry) {
				if(k + needleLength > haystack.length)
					continue;
				if(haystack[k+needleLength-1] != needle[needleStart+needleLength-1])
					continue outer;
