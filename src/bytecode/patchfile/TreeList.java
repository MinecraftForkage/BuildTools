...
public class TreeList<T> extends AbstractList<T> {
	...
	private static <T> List<T> createBalancedTree(List<T>[] chunks, int start, int end) {
		if(end == start + 1)
			return chunks[start];
		
		int mid = (end + start) / 2;
		if(mid <= start || mid >= end) throw new AssertionError();
		return new TreeList<T>(createBalancedTree(chunks, start, mid), createBalancedTree(chunks, mid, end));
	}
	...
	public TreeList(List<? extends T> from) {
		...
		List<T>[] chunks = (List<T>[])new List[numChunks];
		
		for(int chunkstart = 0; chunkstart < fromSize; chunkstart += CHUNKSIZE) {
			int chunkend = Math.min(chunkstart + CHUNKSIZE, fromSize);
			
			chunks[chunkstart / CHUNKSIZE] = new ArrayList<>(from.subList(chunkstart, chunkend));
		}
		...
		if(numChunks == 0) {
			...
			left = createBalancedTree(chunks, 0, chunks.length/2);
			right = createBalancedTree(chunks, chunks.length/2, chunks.length);
		}
